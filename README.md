...
AnyTLS 接收端采用官方 AnyTLS 和 sing-box 相同的同步 stream 模型：每条 stream 对应一个无内部队列的同步 pipe，PSH 数据被该 stream 的处理协程消费后，会话才继续读取下一帧。内存反压直接沿 pipe、TLS 和 TCP 接收窗口传回客户端，不再使用项目自定义的单 stream、单会话或全局缓存预算。上行由 stream 直接向 Xray 提供最大 32 KiB 的 `MultiBuffer`，避免把客户端的大 PSH 重复拆成 8 KiB pipe 读取；下行会把 Xray 的 `MultiBuffer` 连续合并为常规最大 32 KiB 的 PSH 帧（协议上限仍为 65535 字节），避免把每个 8 KiB 内部 buffer 变成独立 TLS 写入而在高 RTT 链路上严重限制 TCP 发送窗口，同时与 Trojan、VLESS 和官方 AnyTLS 的常规复制批次保持一致。超过单帧上限的一次 stream 写入会在会话写锁内连续拆帧，避免其他 stream 的帧插入；每条 stream 的写超时彼此独立，控制帧写入最多等待 5 秒，失去响应的对端不会长期阻塞会话管理。按照 AnyTLS 的常规使用方式，客户端通常为一次代理请求打开一个 session stream；同一 session 中未消费数据的 stream 会阻塞后续帧。用户限速在 UDP 路径上按完整数据报等待令牌，不会为了适配令牌桶 burst 而拆分游戏或 QUIC 数据报。
