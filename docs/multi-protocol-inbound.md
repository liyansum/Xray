...
- Trojan
- VLESS + TCP + TLS + `xtls-rprx-vision`（强制 flow，不接受无 flow VLESS）
...
VLESS 仅接受 TCP 命令和精确的 `xtls-rprx-vision` flow，并要求外层 TLS 1.3。Vision 的 padding、TLS-in-TLS 识别和满足条件后的 raw-copy/splice 路径沿用 Xray-core 的实现。
